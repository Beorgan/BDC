        ...
        <div class="col-sm-12">
            ...
            <div class="card">
                ...
                <div class="card-body text-dark">
                    ...
                    {{-- Forme mise a jour d'un probleme --}}
                    <form action = "/probleme/update/{{ $problems->id}}" method = "post">
                        ...
                        <table>
                            <tr>
                                ...
                                <select class="custom-select" id='Platformes_id' name="Platformes_id" content-type="choices" trigger="true"
                                    ...
                                    @foreach($Platformes as $Platforme) {{-- recuperation de la platforme du probleme  --}}
                                    <option value={{$Platforme->id}} >  {{$Platforme->NomPlatforme}}     </option>
                                    @endforeach
                                    <?php $Platformes = \App\Platformes::where('id', '<>', $problems->Platformes_id)->get()   ?>
                                    @foreach($Platformes as $Platforme) {{-- recuperation du des platformes --}}
                                    <option value={{$Platforme->id}} >  {{$Platforme->NomPlatforme}}   </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="custom-select" id="Serveurs_id" name="Serveurs_id" content-type="choices" trigger="true">
                                    ...
                                    <optgroup label="Serveur" reference={{$Platforme->id}}>
                                        @if ($Platforme->id==$problems->Platformes_id) {{-- Recuperer la platforme du probleme --}}
                                        <?php $Serveurs = \App\Serveurs::where('id', $problems->Serveurs_id)->get()   ?>
                                        @foreach($Serveurs as $Serveur) {{-- Recuperation du serveur de la platforme du probleme --}}
                                        ...
                                        {{--Recuperation des serveurs qui n'apartiennent pas au probléme(le reste) mais appartiennent a la platforme sselectionnée--}}
                        ...
                        {{-- Script chaque platforme a ses propre serveur --}}
                        <script>  
                        /*
